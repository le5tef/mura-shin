<template>
  <div ref="target" class="select-wrapper pointer" @click="active=!active">
    <div  class="select flex justify-space-between" :class="active?'active':''">
      <div :class="!modelValue?'not-selected':''">{{ activeOption }}</div>
      <ArrowIcon class="ml-3" :active="active"/>
    </div>
    <div v-if="active" class="flex flex-column list">
      <div  v-for="option in options" @click="emits('update:modelValue',option.id)" :key="option.id">{{option.label}}</div>
    </div>
  </div>
</template>
...
const activeOption = computed(() => {
  if (props.modelValue) return props.options.find((x) => x.id === props.modelValue).label
