<template>
  <div class="w-full h-100vh app flex-column flex">
    <Header/>
    <div class="w-full flex justify-center mt-6" v-if="label&&uiStore.mdAndUp">
      <div class="container">
...
const breadcrumbs = computed(() => {
  let crumbs = router.currentRoute.value.matched.map((x) => {
    if ('label' in x.meta || 'title' in router.currentRoute.value.query) {
      if (x.name === router.currentRoute.value.name) {
        return {
          label: ('title' in router.currentRoute.value.query) ? router.currentRoute.value.query.title : x.meta.label,
          clickable: false,
          name: x.name
        }
      } else {
