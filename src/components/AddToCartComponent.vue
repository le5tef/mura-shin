<template>
  <div>
    <div class="flex gap-2" v-if="props.isSmall">
      <div class="count flex white-color gap-3 align-center " style="user-select: none">
    ...
    <div class="good-purchase-wrapper" v-else>
      <div class="good-purchase">
        <div class="fw-700 f-32">{{ numberWithSpaces(good.cost*count||good.cost) }} руб.</div>
        <div class="flex justify-space-around align-center mt-4">
          <div class="fw-800 pointer f-20 add-button "  style="user-select: none" @click.stop="add(-1)">-</div>
          <div class="f-20 " style="min-width: 10px">{{ count }}</div>
          <div class="fw-800 pointer f-20 add-button" style="user-select: none" @click.stop="add(1)">+</div>
        </div>
        <div class="relative mt-5 flex justify-end" v-if="showCount">
          <div class="flex gap-2 underline fw-400 f-12 pb-2 w-full text-align-center " style="width: fit-content !important;">
          <span class="primary-color fw-700 ">В наличии</span>{{good.count}} шт.
          </div>
        </div>
      </div>
    </div>
